/* Thermal-printer noise, synthesised — no audio file to ship.
   Three layers: the feed motor hum, the stepper ratchet that gives it the
   grainy "zzzt", and paper hiss. Ends on the little cut-bar clack. */
...
// One second of white noise, reused by every layer at different playback rates.
...
/* Browsers keep the context suspended until a gesture. Call this from a click
   so the next print has sound ready. */
export function primeAudio() {
  const context = audioContext();
  if (context && context.state === "suspended") context.resume().catch(() => {});
}

export function playPrinterSound({ delay = 0, duration = 1.5, volume = 0.5 } = {}) {
  const context = audioContext();
  if (!context) return;
  if (context.state === "suspended") {
    // Not unlocked yet (e.g. the automatic first print) — stay silent.
